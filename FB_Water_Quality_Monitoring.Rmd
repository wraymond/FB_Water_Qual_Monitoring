...
iso <- read.csv("../Data/FBWS_isotope_simp.csv", header = TRUE, stringsAsFactors = FALSE)
```
...
iso$sample_date <- as.POSIXct(iso$sample_date, format = "%m/%d/%Y")
```
...
p.temp <- ggplot(dat %>%
  ...
  scale_color_manual(values = c("#E6003D", "#678F22", "#000000")) +
  labs(color = "Station") +
...
```{r}
## Tidbit ##
png(filename = "../Figures/Temp_Water_Tidbit_2_3.png", width = 6.5, height = 4.8, res = 400, units = "in")
ggplot(tid.daily %>% 
         filter(station_id %in% c("FBWS_2", "FBWS_3"))) +
  geom_line(aes(x = date_short, y = mean_temp), size = 0.5) +
  geom_ribbon(aes(x = date_short, ymin = mean_temp - sd_temp, ymax = mean_temp + sd_temp), alpha = 0.25) +
  geom_point(aes(x = sample_date, y = temp_c, shape = station_abb, color = station_abb), size = 2, data = dat %>% filter(station_id %in% c("FBWS_2", "FBWS_3"))) +
  scale_y_continuous(limits = c(-5, 33), breaks = seq(-0, 30, by = 5)) +
  scale_x_datetime(date_breaks = "month", date_labels = "%b %y", limits = c(as.POSIXct("2021-04-01"), as.POSIXct("2022-06-01"))) +
  scale_shape_manual(values = c(15, 17), labels = c("Bailer", "Mouth", "Nearshore")) +
  scale_color_manual(values = c("#E6003D", "#678F22")) +
  ylab("Temperature (°C)") +
  xlab("Date") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8)) +
  theme(text = element_text(size = 12)) +
...
p.sal <- ggplot(dat %>%
  ...
  scale_shape_manual(values = c(17, 16), labels = c("Mouth", "Nearshore")) +
  scale_color_manual(values = c("#678F22", "#000000")) +
  labs(color = "Station") +
  theme(legend.position = "none") +
  scale_y_continuous(limits = c(0, 36), breaks = seq(0, 36, by = 4))  +
  scale_x_datetime(date_breaks = "month", date_labels = "%b %y", limits = c(as.POSIXct("2021-03-30"), as.POSIXct("2022-09-01"))) +
...
p.precip <- ggplot(fh.monthly) +
  geom_col(aes(x = date, y = total.precip), fill = "#C1C1CC") +
    geom_errorbar(data = fh.monthly.sum, aes(x = date, ymin = sd_min_monthly, ymax = sd_max_monthly), width = 0) +
  geom_point(data = fh.monthly.sum, aes(x = date, y = mean_tot_precip), size = 1.5, pch = 1) +
  scale_y_continuous(limits = c(0, 250), breaks = seq(0, 250, by = 50)) +
  scale_x_datetime(date_breaks = "month", date_labels = "%b %y", limits = c(as.POSIXct("2021-03-30"), as.POSIXct("2022-09-01"))) +
  ylab("Total precipitation (mm)") +
  xlab("") +
...
```{r}
png(filename = "../Figures/Ulva_Precip_Sal_Temp.png", width = 4, height = 8.5, res = 300, units = "in")
...
p.ulva <- ggplot(ul.sum) +
  geom_point(aes(x = date, y = mean_ul_mass_gmsq), size = 1.6, pch = 1) +
  geom_errorbar(aes(x = date, ymin = mean_ul_mass_gmsq - sd_ul_mass_gmsq, ymax = mean_ul_mass_gmsq + sd_ul_mass_gmsq), width = 0) +
  scale_y_continuous(limits = c(-30, 180), breaks = seq(-30, 180, by = 30))  +
  scale_x_datetime(date_breaks = "month", date_labels = "%b %y", limits = c(as.POSIXct("2021-03-30"), as.POSIXct("2022-09-01"))) +
  ylab("Ulvoid mass (g/msq)") +
  xlab("") +
  theme(text = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(plot.margin = unit(c(0, 0, 0 , 0.55), "cm"))
...
```{r}
nut.redu <- nuts.mean %>% 
  filter(station_name %in% c("False Bay Creek Mouth", "False Bay Nearshore")) %>% 
  mutate(date.mo = format(sample_date, "%m %y")) %>% 
  mutate(sample_no = 1:20) %>% 
  filter(sample_no %in% c(1:7, 14:19))

ul.redu <- ul.sum %>% 
  mutate(date.mo = format(date, "%m %y"))

ul.nut <- left_join(nut.redu, ul.redu, by = date.mo)

ggplot(ul.nut) +
  geom_point(aes(x = NO2_umol, y = mean_ul_mass_gmsq))
```
...
```{r}
iso <- iso %>% 
  mutate(mass_per_C = (C_ug / 1000) / CN_sample_mg * 100,
         mass_per_N = (N_ug / 1000) / CN_sample_mg * 100,
         mass_per_O = (O_ug / 1000) / O_sample_mg * 100)

t.test(iso$del_N[iso$sample_date == "11/26/2021"], iso$del_N[iso$sample_date == "5/25/2022"])

t.test(iso$del_O[iso$sample_date == "11/26/2021"], iso$del_O[iso$sample_date == "5/25/2022"])

t.test(iso$mass_per_N[iso$sample_date == "11/26/2021"], iso$mass_per_N[iso$sample_date == "5/25/2022"])

t.test(iso$mass_per_O[iso$sample_date == "11/26/2021"], iso$mass_per_O[iso$sample_date == "5/25/2022"])

iso_sum <- iso %>% 
  group_by(sample_date) %>% 
  summarise(mean_delC = mean(del_C),
            sd_delC = sd(del_C),
            mean_delN = mean(del_N),
            sd_delN = sd(del_N),
            mean_delO = mean(del_O),
            sd_delO = sd(del_O),
            mean_per_C = mean(mass_per_C),
            sd_per_C = sd(mass_per_C),
            mean_per_N = mean(mass_per_N),
            sd_per_N = sd(mass_per_N),
            mean_per_O = mean(mass_per_O),
            sd_per_O = sd(mass_per_O))
