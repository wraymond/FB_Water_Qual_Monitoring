...
dat$station_abb <- factor(dat$station_abb, levels = c("LZ - Inlet", "Emm - Outlet", "LZ - Lower Dam", "FBC - Lower", "SJVC - Upper", "SJVC - Lower", "FBC - Bailer Hill", "FBC - Mouth", "FB - Nearshore"))
```
...
```{r}
fh <- read.csv("../Data/FH_Airport_1.2000_4.2022.csv", header = TRUE, stringsAsFactors = FALSE)
```
...
```{r}
ggplot(fh.monthly) +
  geom_point(aes(x = date, y = total.precip), size = 2) +
  geom_line(aes(x = date, y = total.precip)) +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 1), sec.axis = sec_axis(trans = ~.*24.5, name = "Total precipitation (mm)"))  +
  scale_x_datetime(breaks = seq(as.POSIXct("2021-04-01"), as.POSIXct("2022-04-01"), "month"), date_labels = "%b", limits = c(as.POSIXct("2021-04-01"), as.POSIXct("2022-04-01"))) +
