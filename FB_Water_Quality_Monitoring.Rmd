...
Rename cols
```{r}
upwell <- bind_rows(upwell, upwell2, upwell3)
...
```{r}
#buoy.dat <- buoy(dataset = "stdmet", buoyid = 46088)

#buoy.dat <- as.data.frame(buoy.dat$data)

#test <- ncdc(datasetid = "NORMAL_DLY", stationid = "USW00094276", startdate = "2000-01-01", enddate = "2023-01-01")
```
...
ggplot(dat %>%
  ...
  geom_point(aes(x = sample_date, y = sal_ppt), size = 1) +
...
ggplot(fh.monthly) +
  geom_col(aes(x = date, y = total.precip), fill = "#CCCCCC") +
  geom_line(data = fh.monthly.sum, aes(x = date, y = mean_tot_precip), size = 1, color = "#1E5CD1") +
...
ggplot(fh) +
  ...
  scale_x_datetime(date_breaks = "month", date_labels = "%b %y", limits = c(as.POSIXct("2020-04-01 00:00:00"), as.POSIXct("2022-08-01 00:00:00"))) +
...
ggplot(fh) +
  ...
  scale_x_datetime(date_breaks = "year", date_labels = "%Y", limits = c(as.POSIXct("2012-01-01 00:00:00"), as.POSIXct("2022-08-01 00:00:00"))) +
  scale_y_continuous(limits = c(0, 12), breaks = seq(0, 12, by = 2)) +
  ylab("Mean daily wind speed (m/s)") +
  xlab("Date") +
  theme(text = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust=1))
...
png(filename = "../Figures/LaPush_Upwell_2016_2022.png", width = 11, height = 4, res = 400, units = "in")
ggplot(upwell.month) +
  geom_col(aes(x = date, y = mean_OffTrans_m3s), size = 1) +
  geom_errorbar(aes(x = date, ymin = mean_OffTrans_m3s - sd_OffTrans_m3s, ymax = mean_OffTrans_m3s + sd_OffTrans_m3s), width = 0) +
  geom_hline(aes(yintercept = 0), color = "#8F8F8F") +
  #scale_y_continuous(limits = c(-1000, 250), breaks = seq(-1000, 250, by = 100)) +
  scale_x_datetime(date_breaks = "month", date_labels = "%b %y") +
  ylab("Mean Offshore Transport Index (+/- SD)") +
  xlab("Date") +
  theme(text = element_text(size = 10), axis.text.x = element_text(angle = 45, hjust=1))
dev.off()
```
...
```{r}
## Plots of Means ##
png(filename = "../Figures/Ulva_mass.png", width = 4.5, height = 3, res = 400, units = "in")
ggplot(ul.sum) +
